...
def add_name(names, year, rank, name):
    ...
    {'Abe': {2000: 10}}
    >>> add_name({}, 2014, 89, 'Jason')
    {'Jason': {2014: 89}}
    >>> add_name({}, 1998, 80, 'Jason')
    {'Jason': {1998: 80}}
    """
    if name not in names:
        names[name] = {}
    name_lst = names[name]
    if year not in name_lst:
        name_lst[year] = rank
    return names
...
def parse_year(filename):
    ...
    if dash != -1 and period != -1:
        year = int(filename[dash + 1: period])
        return year
    else:
...
def add_file(names, filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            man = split[1]
            woman = split[2]
            add_name(names, year, rank, man)
            add_name(names, year, rank, woman)
    return names
...
def search_names(names, target):
    ...
    Not case sensitive.
    (Code and tests TBD)
    >>> search_names(['Jason', 'Jeannette', 'Jeffrey'], 'Ja')
    ['Jason']
    >>> search_names(['Binny', 'Billy', 'Bobby'], 'Bi')
    ['Billy', 'Binny']
    >>> search_names(['jeff', 'Jalapenoo', 'Hoolland', 'mary', 'YOURMOOM', 'TOODLES', 'BOOMSHAKALAKA'], 'oo')
    ['BOOMSHAKALAKA', 'Hoolland', 'Jalapenoo', 'TOODLES', 'YOURMOOM']
    """
    lst = []
    for name in names:
