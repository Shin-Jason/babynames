...
def compute_x(width, year_index):
    ...
    """
    return SPACE + (year_index * width/len(YEARS))
...
def draw_fixed(canvas):
    ...
    for i in range(len(YEARS)):
        canvas.create_line(compute_x(width-SPACE*2, i), 0, compute_x(width-SPACE*2, i), height)
        canvas.create_text(compute_x(width-SPACE*2, i), height-SPACE, text=YEARS[i], anchor=tkinter.NW, fill='red')
...
def compute_rank(names, name, year):
    ...
    """
    if name in names:
        # checks if it's in key
        # checks Abe, if Abe is there
        name_rank = names[name]
        # access value of key Abe
        # checks if in the key of key

        if year in name_rank:
            return name_rank[year]
    return MAX_RANK
...
def compute_y(names, name, height, year_index):
    ...
    return SPACE + ((compute_rank(names, name, year) / (MAX_RANK)) * height)
...
def draw_name(canvas, names, name, color):
    ...
    for i in range(len(YEARS)):
        x1 = compute_x(width - 2 * SPACE, i)
        y1 = compute_y(names, name, height - 2 * SPACE, i)
        x2 = compute_x(width - 2 * SPACE, i - 1)
        y2 = compute_y(names, name, height - 2 * SPACE, i - 1)
        if i > 0:
            canvas.create_line(x2, y2, x1, y1, fill=color)
        canvas.create_text(x1 + TEXT_DX, y1 - SPACE, text=(name, compute_rank(names, name, YEARS[i])), anchor=tkinter.NW,
                           fill=color)
...
def draw_names(canvas, names, lookups):
    ...
    for string in range(len(lookups)):
        draw_name(canvas, names, lookups[string], COLORS[string % len(COLORS)])
